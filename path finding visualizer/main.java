import java.awt.Graphics; // used to draw or paint on the components
import java.awt.Color; //The Color class creates color by using the given RGBA values where RGBA stands for RED, GREEN, BLUE, ALPHA or using HSB value where HSB stands for HUE, SATURATION, BRIcomponents. The value for individual components RGBA ranges from 0 to 255 or 0.0 to 0.1. The value of alpha determines the opacity of the color, where 0 or 0.0 stands fully transparent and 255 or 1.0 stands opaque.
import java.awt.event.ActionEvent; //An action event is a semantic event which indicates that a component-defined action occurred. - The ActionListener interface gets this ActionEvent when the event occurs. - Event like Button pressed is an action event. - It is defined in 'java.awt.event' package.
import java.awt.event.ActionListener; //The Java ActionListener is notified whenever you click on the button or menu item. It is notified against ActionEvent. 
import java.awt.event.ItemEvent; //public class ItemEvent extends AWTEvent A semantic event which indicates that an item was selected or deselected.
import java.awt.event.ItemListener; //ItemListener mainly used for item selection, and it is an interface that listens for the item event. The interface ItemListener basically manages the on or off status for one or several items. The Java ItemListener notifies each and every click on the checkbox, and it let you know against the ItemEvent.
import java.awt.event.MouseAdapter; //Mouse events let you track when a mouse is pressed, released, clicked, moved, dragged, when it enters a component, when it exits and when a mouse wheel is moved.
...
public class main {
	...
	private final int MSIZE = 550;
	private int CSIZE = MSIZE/cells;

	//UTIL ARRAYS
	private String[] algorithms = {"Dijkstra","A*","Greedy BFS","BFS", "DFS"};
	...
	public void resetMap() {	//RESET MAP
		...
		if(startx > -1 && starty > -1) {	//RESET THE START AND FINISH
			...
			map[startx][starty].setHops(0); //see once set hops
		}
	...
	private void initialize() {	//INITIALIZE THE GUI ELEMENTS
		...
		toolP.setBounds(10,10,210,600); //of the controls panel
		
		searchB.setBounds(40,space, 120, 25); // of start search button
		toolP.add(searchB);
		space+=buff;
		...
		searchB.addActionListener(new ActionListener() {		//ACTION LISTENERS //when the start search bar is clicked
			@Override
			public void actionPerformed(ActionEvent e) {
				reset();
				if((startx > -1 && starty > -1) && (finishx > -1 && finishy > -1))
					solving = true; //solving is set to true
			}
		});
	...
	public void pause() {	//PAUSE STATE puses when it traverses all the cells, max is 500, as we have set the max. no of cells to 500
		int i = 0;
		while(!solving) {
			i++;
			if(i > 500)
				...
				for(int y = 0; y < cells; y++) {
					switch(map[x][y].getType()) {
						case 0:
							...
							break;
						case 1:
							g.setColor(Color.RED); //finish node
							break; 
						case 2:
						...
						case 3:
							...
							break;
						case 4:
							g.setColor(Color.CYAN); //color of the grid as it is visited
							break;
						case 5:
							g.setColor(Color.YELLOW); //path color
							break;
					}
					g.fillRect(x*CSIZE,y*CSIZE,CSIZE,CSIZE);
					g.setColor(Color.yellow); //grid border
